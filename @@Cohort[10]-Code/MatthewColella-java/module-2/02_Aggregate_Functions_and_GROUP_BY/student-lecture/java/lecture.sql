...
SELECT name, population FROM country ORDER BY population, name;
...
-- Population density in Ascending order
SELECT name, population/surfacearea FROM country ORDER BY 2;
--------------------------------------------------------------------------------------------------------
...
SELECT name, lifeexpectancy FROM country WHERE lifeexpectancy IS NOT NULL ORDER BY lifeexpectancy DESC LIMIT 30;
...
SELECT name || ', ' || district AS Location FROM city WHERE district IN('California', 'Oregon', 'Washington') ORDER BY population, district, city;
SELECT name || ', ' || district AS Location FROM city WHERE district IN('California', 'Oregon', 'Washington') ORDER BY population, district, city;
...
SELECT name, indepyear  FROM country WHERE indepyear = '1991';
--------------------------------------------------------------------------------------------------------
...
select language, count(*) from countrylanguage group by language order by count DESC;


-- Show the average life expectancy of each continent ordered from highest to lowest
select continent, avg(lifeexpectancy) from country where lifeexpectancy IS NOT NULL group by continent;
...
select name, countrycode, district, population from city where countrycode in (SELECT code FROM country WHERE headofstate = 'Jacques Chirac');
...
select name, countrycode, district, population from city where countrycode in(select code from country where indepyear IS NULL);
--------------------------------------------------------------------------------------------------------
--
-- Additional samples
--
