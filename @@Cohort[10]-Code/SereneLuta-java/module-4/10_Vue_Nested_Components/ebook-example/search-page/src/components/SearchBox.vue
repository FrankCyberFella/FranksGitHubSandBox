...
export default {
    ...
    methods: {
         ...
         */
        performSearch() {
            //fetch() - go to the searchUrl API with the search value and the parameter to request data in json format
            //.then - wait for the response from the API and then return the json data we got
            return fetch(this.searchUrl + this.search + '?format=json').then((response) => {
                 ...
                 */
                this.$emit('search-results', results); //notify the parent we got the data requested
            });
        }
    }
}
