...
<template>
    <div class="main">
        ...
        <div class="review" v-for="review in reviews" v-bind:key="review.id">
            ...
            </div>                            <!-- binding a title to the image -->           <!-- give me the number of images indicated by the rating (Repeats the image 'n' number of times -->
            <h3>{{ review.title }}</h3>
...
export default {
    ...
    computed: {
        ...
        numberOfOneStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 1);//call the number of Reviews method passing the array of views and a number
        },                                           //the number of review with the rating of 1
        numberOfTwoStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 2);//call the number of Reviews method passing the array of views and a number
        },                                           //the number of review with the rating of 2
        numberOfThreeStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 3);//call the number of Reviews method passing the array of views and a number
        },                                           //the number of review with the rating of 3
        numberOfFourStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 4);//call the number of Reviews method passing the array of views and a number
        },                                           //the number of review with the rating of 4
        numberOfFiveStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 5);//call the number of Reviews method passing the array of views and a number
        },                                           //the number of review with the rating of 5
    },
    methods: {
        numberOfReviews(reviews, starType) {
            return reviews.reduce( (currentCount, review ) => { //will for loop the array and reduce to one value
                return currentCount + ( review.rating === starType ? 1 : 0); //if true add 1, if false add 0
            }, 0);
        }
    }
}
