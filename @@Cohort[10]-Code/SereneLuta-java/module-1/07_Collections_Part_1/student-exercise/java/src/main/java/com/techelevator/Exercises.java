...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		// define a variable or object to hold a return type
		List<String> result = new ArrayList<String>();
		for(String newString : stringArray) {
			result.add(newString);
		}
		// identify the input (you know what the output is)
		// figure out how to make the output from the input
		
		// output: ArrayList of Strings
		// input: array of Strings
		// how to take an array of Strings into an ArrayList of Stings
		return result;  // remember to return the object/variable defined to hold result
	}
	...
	public String[] list2Array(List<String> stringList) {
		...
		int i = 0;
		for(String anElement: stringList) {
			newArray[i] = anElement;
			i++;
		}
		return newArray;
	}
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for(String anElement: stringArray) {
			if(anElement.length() != 4)
			{
				newArrayList.add(anElement);
			}
		}
		return newArrayList;
	}
	...
	public List<String> reverseList(List<String> stringList) {
		...
		for(String color: stringList) {
			stringStack.push(color);
		}
		
		while(stringStack.isEmpty() == false) {
			String color = stringStack.pop();
			reverseList.add(color);
		}
		return reverseList;
	}
	...
	public Integer findLargest(List<Integer> integerList) {
		int x = Collections.max(integerList);
...
//		int[] oddList = new int[integerArray.length()];
		List<Integer> oddList = new ArrayList<Integer>();
		
		for(Integer anElement : integerArray) {
			if(anElement % 2 != 0)
			{
				oddList.add(anElement);
			}
		}
		return oddList;
	}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		 boolean bool = false;
		    int i = 0;
		    for (Integer anElement : integerList) {
		        if (anElement == intToFind) {
		            i++;
		        }
		    }
		    if (i > 1){
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> listCombo = new ArrayList <>();
		int smallerLength; //hold the number of items in the smaller list
		...
		for (i = 0; i < smallerLength; i++) { //when i exits the foreach loop it will = 1+ the last length of the smallest ArrayList
			listCombo.add(listOne.get(i));
			listCombo.add(listTwo.get(i));
		}
		if(listOne.size() > listTwo.size()) {
			for(int j = i ;j < listOne.size(); j++) {
				listCombo.add(listOne.get(j));
			}
		}
		else {
			if(listTwo.size() > listOne.size()) {
				for (int k = i; k < listTwo.size(); k++) {
					listCombo.add(listTwo.get(k));
				}
