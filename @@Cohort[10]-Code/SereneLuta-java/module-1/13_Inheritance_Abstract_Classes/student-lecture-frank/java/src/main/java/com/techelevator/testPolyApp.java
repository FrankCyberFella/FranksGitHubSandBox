...
//		Worker aWorker    = new Worker("Werk", "Kerr"); // cannot instantiate a new worker b/c it is an abstract class
...
//		showTest(aWorker   , 50);   // expecting -1
	...
	public static void showTest(Worker aWorker, int hoursWorked) {		
		...
		System.out.printf("%s %s working for %d hours makes $%.2f per week\n", 
		                 aWorker.getFirstName(), // replace the 1st placeholder "%s" = String
		                 aWorker.getLastName(), // replace the 2nd placeholder "%s" = String
		                 hoursWorked,           // replace the 3rd placeholder "%d" = int value
		                 aWorker.calculateWeeklyPay(hoursWorked)); // replace the 4th placeholder and rounds it to 2 decimal places 0.00
