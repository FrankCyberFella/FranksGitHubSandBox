...
public class HelloController {
	...
	public String displayHomePage() {
		...
		return "homePage";   // logical view name - a jsp
	}
	...
	public String displayGreeting(HttpServletRequest request) {
		...
		String name = request.getParameter("name");   // get value form the name= on the query string
		Greeter greeter = new Greeter(name);          // instantiate a Greeter object using the value from query string
		request.setAttribute("greeting", greeter.getGreeting());  // Add the value from getGreeting() as an entry in the 
		                                                          //    request map with the key greeting
		...
		return "greeting";    // logical view name - jsp
	}
}
