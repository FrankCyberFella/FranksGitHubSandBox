...
public class Exercises {
	...
	public int[] reverse3(int[] nums) {
		...
		int[] rev = new int[3];
		// numbs = {nums[1],nums[2],nums[0]};
		// int[] numbs = {nums[1],nums[2],nums[0]};
		rev[0] = nums[2];
		rev[1] = nums[1];
		rev[2] = nums[0];
		return rev;
	 ...
	 */
	/*
	 * public int[] maxEnd3(int[] nums) { if (nums[0] >= nums[2]) { nums[2] =
	 * nums[0]; nums[1] = nums[0]; return nums;
	 * 
	 * } else if (nums[2] >= nums[0]) { nums[0] = nums[2]; nums[1] = nums[2];
	 * nums[1] = nums[2]; } return nums; }
	 */
	 ...
	 * 3's. lucky13([0, 2, 4]) → true lucky13([1, 2, 3]) → false lucky13([1, 2, 4])
	 * → false
	 */
