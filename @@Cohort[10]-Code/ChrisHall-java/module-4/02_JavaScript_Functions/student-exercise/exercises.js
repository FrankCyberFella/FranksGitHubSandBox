 ...
 * @param {string} [second=''] the second string of digits to concatenate   first, second=''
 ...
 * 
 */
// function addAll(...num) {                                //  ********    this works
//   return num.reduce((total, arg) => total + arg, 0);
//   }
  ...
  //   function addAll(...num) { 
  //     let result = 0;
  //     for(let i = 0; i < arguments.length; i++) {     // ******     this works too
  //         result += arguments[i];
  //     }
  //     return result;
  // }
...
function getFullAddressesOfProperties(array) {
  return array.map( (assocArray) => {
    return assocArray.streetNumber + ' '+ assocArray.streetName +' ' + assocArray.streetType +' '+ assocArray.city +' ' + assocArray.state +' ' + assocArray.zip;
  })
}
/*
 ...
 */
function findLargest(array) {      // why doesn't this work for numbers??
  let maxValue = '';
  array.forEach( (anElement) => {
    if(maxValue < anElement) 
    maxValue = anElement;
  });
  return maxValue;
}
