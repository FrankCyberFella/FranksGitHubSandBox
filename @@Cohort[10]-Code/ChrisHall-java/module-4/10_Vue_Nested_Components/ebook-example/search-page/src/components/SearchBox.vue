...
<!-------------------------------------------------------------->
<template>
    <form v-on:submit.prevent="performSearch">  <!-- on submit, don't do default action, run performSearch method instead -->
        <input type="text" name="search" id="search" v-model="search"> <!-- create a text box, v-model says two way binding -->
...
export default {
    ...
    methods: {
         ...
         */
        performSearch() {  // fetch() method, goes to searchUrl api, pass the search field as an arg and request
        // the data in json format.  .then says "wait for response to the api" --then return json data.
            return fetch(this.searchUrl + this.search + '?format=json').then((response) => {
                return response.json();
            }).then((results) => { //says: after we process the response, issue a custom event ($emit)
                /**
                 ...
                 */
                this.$emit('search-results', results); // notify the parent that we got the data.
            });
        }
    }
}
