...
insert into countrylanguage
        (countrycode, language, isofficial, percentage)  -- list columns for data you want to add
...
-- ***  good to do the above rollback as a check to see if it works, rollback, before making the change
BEGIN TRANSACTION;
...
-- UPDATE
--  **** ALWAYS code a WHERE condition with UPDATE ***********************

-- 1. Update the capital of the USA to Houston ????
BEGIN TRANSACTION

update country
set capital = 3796  --  or: = (Select id from city where name = 'HOUSTON')  --subquery to find code for Houston
where code = 'USA'
;
...
--update multi
s --  Select capital, headofstate from country
-- DELETE  ----  also remember WHERE clause  ******************
...
insert into countrylanguage
(countrycode, language, isofficial, percentage0 
values('CHN', 'Elvish', true, 1000)  --primary key constraint
;
-- 2. Try inserting English as a spoken language in the country ZZZ. What happens?
insert into countrylanguage
(countrycode, language, isofficial, percentage0
values('ZZZ', 'Elvish', true, 1000  -- foreign key constraint
;
-- 3. Try deleting the country USA. What happens?
delete from country
where code = 'USA'   --   foreign key constraint
;
