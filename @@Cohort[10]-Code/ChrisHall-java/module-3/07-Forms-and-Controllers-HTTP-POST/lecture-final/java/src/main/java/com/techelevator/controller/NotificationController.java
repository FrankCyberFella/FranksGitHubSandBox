...
public class NotificationController {
	 ...
	 * 
	 *     @RequestParam(name="first_name") String firstName ...  (here the name= is referring to the form variable name in the jsp form.)
	 */
	@RequestMapping(path="/notificationSignupInput", method=RequestMethod.POST)
	public String processNotificationSignupSubmission(@RequestParam String firstName, // to get the form variables from the request body on a POST
													 @RequestParam String lastName,
													 @RequestParam String emailAddress,
													 @RequestParam int age) throws FileNotFoundException, IOException {  //note the automatic conversion of the string to an int
		
		try(FileOutputStream out = new FileOutputStream(getSignupLogFile(), true)) {
			NotificationSignupDAO log = new NotificationSignupDAO(out);     // take data from view, logs parameters to a file, using a DAO to update model
