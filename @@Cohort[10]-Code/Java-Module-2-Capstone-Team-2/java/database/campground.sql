...
SELECT campground_id, c.name, open_from_mm, open_to_mm, daily_fee 
...
Select site.site_number, site.max_occupancy, accessible, max_rv_length, utilities, daily_fee,reservation_id, from_date, to_date
FROM site
        full outer join
        reservation
...
WHERE campground.campground_id = 1 and reservation_id IS null or (reservation.from_date  not between '2019-04-03' and  '2019-05-03') AND
(reservation.to_date not between '2019-04-03' and  '2019-05-03')
LIMIT 5;

-- query currently in Campground Cli
Select site.site_id, site.max_occupancy, accessible, max_rv_length, utilities, campground.name
FROM site
        full outer join
        reservation
      on site.site_id = reservation.site_id
        ...
        campground
      on site.campground_id = campground.campground_id
WHERE campground.name = 'The Unnamed Primitive Campsites' and reservation_id IS null or  campground.name = 'The Unnamed Primitive Campsites' and (reservation.from_date  not between '2019-04-03' and  '2019-05-03') AND
(reservation.to_date not between '2019-04-03' and  '2019-05-03')
GROUP BY site.site_id, site.max_occupancy, accessible, max_rv_length, utilities, campground.name
ORDER BY 1 ASC 
LIMIT 5;

-- info from campgrounds/sites
...
From site
 ...
 select cast(month(dateField) as varchar)
