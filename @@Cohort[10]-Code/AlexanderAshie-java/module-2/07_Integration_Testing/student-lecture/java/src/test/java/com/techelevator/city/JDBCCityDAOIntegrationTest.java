...
public class JDBCCityDAOIntegrationTest {
	...
	public void save_new_city_and_read_it_back() throws SQLException {
		...
		dao.save(theCity); //testing save() in JDBCCityDAO
		City savedCity = dao.findCityById(theCity.getId());
		
		assertNotEquals(null, theCity.getId()); //Ensure the row is in the table
		assertCitiesAreEqual(theCity, savedCity); //Verify that city we retrieved is the same as what we expect
	}
	...
	public void returns_cities_by_country_code() {
		...
		dao.save(theCity);
		List<City> results = dao.findCityByCountryCode(TEST_COUNTRY); //method returns a list
		
		assertNotNull(results); //Ensure the row is in the table
		assertEquals(1, results.size()); //Make sure there is only one row
		City savedCity = results.get(0); //Get the first city object in the list that is returned by find city
		assertCitiesAreEqual(theCity, savedCity); //verifying that city object we retrieved is equal
	}
