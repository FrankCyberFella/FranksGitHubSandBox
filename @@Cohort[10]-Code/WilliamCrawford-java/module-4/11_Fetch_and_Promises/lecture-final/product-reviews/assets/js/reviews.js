...
function loadReviews() {
  ...
  fetch('data.json') // go get the data from this source
    .then((response) => {  // wait until the data is retrieved
      return response.json(); //covert data to json and return it
    })
...
function displayReviews() {
  ...
  if('content' in document.createElement('template')) {    // if there is a template in the HTML
    ...
    reviews.forEach((review) => {                          // loop through each element 
      const tmpl = document.getElementById('review-template').content.cloneNode(true); // get a reference to the review-template
      ...
      tmpl.querySelector('.published-date').innerText = review.publishedOn;//         whatever esle
