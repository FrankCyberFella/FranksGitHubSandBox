...
<!-------------------------------------------------------------->
<template>
    <form v-on:submit.prevent="performSearch"> <!-- on submit don't do default action and run performSearch method instead -->
...
export default {
    ...
    methods: {
         ...
         */
        performSearch() {
            // fetch() - got to the searchUrl API with the search value and the parameter to request data in 
            return fetch(this.searchUrl + this.search + '?format=json').then((response) => {
                return response.json();
            }).then((results) => { // after we process the response - issue a custion event ($emit)
                /**
