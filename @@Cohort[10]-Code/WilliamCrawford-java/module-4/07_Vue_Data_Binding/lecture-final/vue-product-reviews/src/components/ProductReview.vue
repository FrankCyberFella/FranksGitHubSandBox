        ...
        <div class="review" v-for="review in reviews" v-bind:key="review.id"> 
            ...
            <div class="rating">
                <img src="../assets/star.png" v-bind:title="review.rating + ' Star Review'" class="ratingStar" v-for="n in review.rating" />
                                              <!-- adding a title to image using v-bind:title-->  <!-- give me the number of images indicated buy the rating -->
            </div>
            <h3>{{ review.title }}</h3>
...
export default {
    ...
    computed: {  //new data created by calculation
        averageRating(vm) { //vm is the current component
            let sum = vm.reviews.reduce( (currentSum, review) => {
                return currentSum + review.rating;
            }, 0); // initial value in currentSum to be 0 rather than the value in the first element of the array
            return sum / vm.reviews.length; // divide the sum of all the star ratings by the number of reviews
        },
        numberOfOneStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 1);// call the numberOfReviews method passing the array of reviews and a number
        },                                           // the number of reviews with the rating of 1
        numberOfTwoStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 2);// call the numberOfReviews method passing the array of reviews and a number
                                                     // the number of reviews with the rating of 2
        },
        numberOfThreeStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 3);// call the numberOfReviews method passing the array of reviews and a number
        },                                           // the number of reviews with the rating of 3
        numberOfFourStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 4);// call the numberOfReviews method passing the array of reviews and a number
        },                                              // the number of reviews with the rating of 4
        numberOfFiveStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 5);// call the numberOfReviews method passing the array of reviews and a number
        },                                          // the number of reviews with the rating of 5
    },
    methods: {
        numberOfReviews(reviews, starType) { // receive an array and a number that represents starType
