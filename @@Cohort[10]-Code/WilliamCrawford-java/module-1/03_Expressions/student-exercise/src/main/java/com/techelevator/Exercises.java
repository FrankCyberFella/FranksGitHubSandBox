...
public class Exercises {
	 ...
	 * same, then return double their sum. sumDouble(1, 2) → 3 sumDouble(3, 2) → 5
	 * sumDouble(2, 2) → 8
	 */
	 ...
	 * negative. posNeg(1, -1, false) → true posNeg(-1, 1, false) → true posNeg(-4,
	 * -5, true) → true
	 */
	 ...
	 * multiple of 5. (Hint: Think "mod".) or35(3) → true or35(10) → true or35(8) →
	 * false
	 */
	 ...
	 * 10..20 inclusive, or return 0 if neither is in that range. max1020(11, 19) →
	 ...
	 * successful, or false otherwise. cigarParty(30, false) → false cigarParty(50,
	 * false) → true cigarParty(70, true) → true
	 */
...
//				}
		if (isSummer == true) {
			// It is summer, and the range is 60 -> 100 (inclusive)
			if ((temp >= 60) && (temp <= 100)) {
				return true;
			}
			else {
			...
			// Not summer, and the range is 60 -> 90 (incluseive)
			if ((temp >= 60) && (temp <= 90)) {
				return true;
			}
			else {
	 ...
	 * higher in all cases. caughtSpeeding(60, false) → 0 caughtSpeeding(65, false)
	...
	public String alarmClock(int day, boolean vacation) {
		// if vacation is true
			// if day is saturday or sunday then it's the weekend
				// return "off"
			// else day must be a weekday
				// return "10:00"
		// else vacation must be false so we are not on vacation
			// if day is saturday or sunday then its the weekend
				// return "10:00"
			// else day must a weekday
				// return "7:00"
		if(vacation == true) {
			if(day == 0 || day == 6) {
				return "off";
	...
	public boolean answerCell(boolean isMorning, boolean isMom, boolean isAsleep) {
		// if isAsleep is true (you are asleep)
			// return false because you never answer the phone when you're asleep
		// else
			// if isMorning is true
				// if isMom is true
					// return true because you only answer in morning when its mom
				// else
					// return false because don't answer in morning unless its mom
			// else
				// return true because you weren't asleep, and it is not morning 
		if(isAsleep == true) {
			return false;
		}else {
			if(isMorning == true) {
	...
	public int teaParty(int tea, int candy) {
		// if tea < 5 OR candy < 5
			// return 0 because there wasn't enough tea or candy to make a party
		// else if tea / candy >= 2 OR candy / tea >= 2
			// return 2 because there was twice as much tea as candy, or vice versa
		// else
			// return 1 because there enough tea and candy in more-or-less equal balance
		if(tea < 5 || candy < 5) {
			return 0;
		}else {
