...
// if want to do using loop:
	//int forward = 0;              index of first element
	//int backward = nums.length-1; index of last element
	//for (int i = forward; i < nums.length / 2; i++) {
	//     holdValue = nums[i];
	//     nums[i] = nums[backward];
	//     num[backward] = holdValue;
	//     backward--;               move backward to previous element
	//     }
	//     nums[2] = holdValue;
	//     return nums;
	//     }
	...
	public int[] maxEnd3(int[] nums) {
		...
		if(firstElem > lastElem) {
			return nums[0] = firstElem, nums[1] = firstElem, nums[2] = firstElem;
		return new int[] {};
	}
	...
	public int sum2(int[] nums) {
		...
		if(nums.length < 2) {
				...
				for (int i = 0; i < nums.length; i++) {
					sumAllElements = sumAllElements + nums[i];
				}
	...
	for (int i = 0; i < nums.length-1; i++) {
		...
		firstTwo = firstTwo + nums[i];
		secondTwo = secondTwo + nums[k];
		}
	}
	return nums;
	
	if(firstTwo == 2 && secondTwo == 2) {
		return true;
	}
	return false;
	}
