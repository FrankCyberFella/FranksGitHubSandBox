...
SELECT name 
        FROM country 
                WHERE continent = 'Oceania' and lifeexpectancy isnull
                ;
...
SELECT name , continent , GNP , lifeexpectancy
        FROM country 
                WHERE lifeexpectancy >= 70 and gnp between 1000000 and 100000000
                ;
...
SELECT name, (gnp / population) as per_capita_gnp, continent
        FROM country 
                WHERE continent = 'Europe'
                ;
...
SELECT name, (2019 - indepyear) as years_since_last_independence 
        FROM country 
