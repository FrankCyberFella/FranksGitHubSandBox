...
public class ProductController {
	...
	@RequestMapping("/")											// URL path this code will handle
	public String handleRootPath(HttpServletRequest request) {	    // receive the request object from the server
		request.setAttribute("productList", getProducts());			// create an entry in the request map with w/e getProducts() returns
		return "productList";										// run the productList.jsp
	}																// return the name of the .jsp file to run for this controller as a String
		
	
	@RequestMapping("/productList")	// URL path this code will handle
	public String showProductList(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productList";
	}
	
	@RequestMapping("/productTiles")	// URL path this code will handle
	public String showProductTiles(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productTiles";
	}
	
	@RequestMapping("/productTable")	// URL path this code will handle
	public String showProductTable(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productTable";
	}
	
	@RequestMapping("/productDetail")	// URL path this code will handle
 ...
 // *** for purposes of this example - the model is an ArrayList --> but it can be other things ***
