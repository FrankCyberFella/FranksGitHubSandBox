...
public class Exercises {
	...
	public Double isItOnSale(String itemNumber) {
		...
		result.put("kitchen4001", 0.20);
		result.put("garage1070", 0.15);
		result.put("livingroom", 0.10);
		result.put("bedroom3434", 0.60);
		result.put("bath0073", 0.15);
		// result.put("", 0.00);
		// result.put("dungeon9999", 0.00);

		if (result.containsKey(itemNumber.toLowerCase())) {
			return result.get(itemNumber.toLowerCase());
		}
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if (peterPaul.get("Peter") > 0 && peterPaul.get("Paul") < 1000) {
			int moneyChange = (peterPaul.get("Peter") / 2);
			// peterPaul.put("Paul", (peterPaul.get("Paul") + peterPaul.get("Peter") / 2));
			// peterPaul.put("Peter", (peterPaul.get("Peter")/2));
			peterPaul.put("Paul", (peterPaul.get("Paul") + moneyChange));
			if (peterPaul.get("Peter") % 2 != 0) {
				peterPaul.put("Peter", moneyChange + 1);
			} else
				peterPaul.put("Peter", moneyChange);
			return peterPaul;
		} else {
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for (String i : words) {
			...
			String last = String.valueOf(i.charAt(i.length() - 1));

			characters.put(first, last);
		}
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
		...
		for (int i = 0; i < consolidate.size(); i++) {
			if (!consolidate.containsKey(remoteWarehouse)) {
