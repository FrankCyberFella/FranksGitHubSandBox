...
public class RequestAuthProvider implements AuthProvider {
    ...
    @Autowired
    public RequestAuthProvider(HttpServletRequest request, UserDao dao, RestaurantDao rdao) {
    ...
    @Override
    public void register(String username, String password, String role, String city, String state, String current_preference) {
        dao.saveUser(username, password, role, city, state, current_preference);
    }
    
    //restaurant.getName(), restaurant.getCuisine(), restaurant.getPrice(), restaurant.getRating(), restaurant.getAddress(), restaurant.getLocality(), restaurant.getCity(), restaurant.getMenu());
//    @Override
