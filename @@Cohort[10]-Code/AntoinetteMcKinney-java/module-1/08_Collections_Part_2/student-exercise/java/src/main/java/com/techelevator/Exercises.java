...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		Map<String, String> animalGroupName = new HashMap<String, String>();//declare HashMap
		animalName = animalName.toLowerCase();//Adding elements to HashMap
	...
	public Double isItOnSale(String itemNumber) {
		Map<String, Double> isItOnSale = new HashMap<String, Double>();//declare HashMap
		itemNumber = itemNumber.toLowerCase();//Adding elements to HashMap
		isItOnSale.put("kitchen4001", 0.20);//The "put" method is used to add elements to a Map
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		int halfPeter = peterMoney / 2; //calculate half of peters money incase you need it. 
		
		if((peterMoney > 0) && (paulMoney < 1000)) {
			peterMoney = peterMoney - halfPeter;
			paulMoney = paulMoney + halfPeter;
			peterPaul.put("Peter", peterMoney);//using put to give the money back to 
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		int paulMoney = peterPaul.get("Paul");//get pauls money from the map
		int peterMoney = peterPaul.get("Peter");//get peters money from the map
		int peterContribute = peterMoney / 4; //calculate a quarter of peters money incase you need it. 
		int paulContribute = paulMoney / 4; // calculate a quarter of pauls money
		
		
		
		if((peterMoney >= 5000) && (paulMoney >= 10000)) {
			peterPaul.put("PeterPaulPartnership", peterContribute + paulContribute);
			peterPaul.put("Peter", peterMoney - peterContribute);
			peterPaul.put("Paul", paulMoney - paulContribute);
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for(int i = 0;  i < words.length; i++) {
			String word = words[i];
				beginningAndEnding.put(word.substring(0,1), word.substring(word.length()-1));//word.length we dont know how long the word is. minus two bec going one positon from end
		}
	 ...
	 *
	 */
	public Map<String, Integer> wordCount(String[] words) {//turoring
		//String count = "";
	...
	public List<String> distinctValues(List<String> stringList) {
		for(int i = 0; i < stringList.size(); i++) {
		    System.out.println("distinctValues: " + stringList.get(i));
		}
		return stringList;
	}
