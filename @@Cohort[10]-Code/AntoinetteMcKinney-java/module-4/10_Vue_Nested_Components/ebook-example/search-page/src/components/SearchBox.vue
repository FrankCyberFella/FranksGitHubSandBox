...
<!-------------------------------------------------------------->
<template>
    <form v-on:submit.prevent="performSearch"> <!--on submit dont do default action, run performSearch method instead-->
        <input type="text" name="search" id="search" v-model="search"> <!--defining input box and two-way binding-->
...
export default {
    ...
    methods: {
         ...
         */
        performSearch() {
            // fetch() - go to the searchURL API with the search value and the parameter to request data in json FormData
            ...
            return fetch(this.searchUrl + this.search + '?format=json').then((response) => {
            ...
            }).then((results) => { // after we process the response - issue a custom event($) thats waht the dollarsign doses
                /**
                 ...
                 */
                this.$emit('search-results', results); // notify the parent we go the data requested
            });
        }
    }
}
