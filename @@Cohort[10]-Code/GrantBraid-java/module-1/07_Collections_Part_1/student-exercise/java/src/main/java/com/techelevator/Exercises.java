...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		// identify the input
		// figure out how to make the output from input
		
		// output: ArrayList of Strings
		// input : array of strings
		
		for(String name : stringArray) {
			fruits.add(name); // adding name to ArrayList fruits
		}
		return fruits;
	}
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {//**********
		List<Integer> result = new ArrayList<Integer>();
		int i = 1; //start at index one for my first case since adding to listTwo to listOne
		...
		if (listOne.size() >= listTwo.size()) {
			...
			result = listTwo;
			i = 0; // start at index 0 for this case
			for(int num : listOne) {
				result.add(i, num);
				i += 2;
			}
		...
		/*
		 List<Integer> listCombo = new ArrayList<Integer>(); //Define a list to hold result
		    // Go through both lists for number of items
		    ...
		    int j = 0;
		    // interleave listCombo the same number of elements - size are equal
		   for (j = 0; j < smallerLength ; j++) { // loop for the # of items of listOne
		    
		    	listCombo.add(listOne.get(j)); // get an item from listOne and add it to listCombo
		    	listCombo.add(listTwo.get(j)); // get an item from listTwo and add it to listCombo
		    
		    }
		    
		    if (listOne.size() > listTwo.size()) {
		    	// add the remaining elements from listOne to listCombo
		    	for (; j < listOne.size(); j++) {
		    	listCombo.add(listOne.get(j));
		    	}
		    }
		    else { // listTwo is bigger or same size
		    	if (listTwo.size() > listOne.size()) {
		    	for (; j < listTwo.size(); j++) {
		    	listCombo.add(listTwo.get(j));
		    	}
