        ...
        <div class="review" v-for="review in reviews" v-bind:key="review.id">
            ...
            <div class="rating">
                <img src="../assets/star.png" v-bind:title="review.rating + ' Star Review'" class="ratingStar" v-for="n in review.rating" />
                                 <!-- adding a title to image using v-bind:title -->
            </div>
            <h3>{{ review.title }}</h3>
...
export default {
    ...
    computed: {
        numberOfStars(vm) {
            let sum = vm.reviews.reduce( (sum, review) => {
    ...
    methods: {
        numberOfReviews(reviews, starType) {            // receive an array and a number the represents starType
