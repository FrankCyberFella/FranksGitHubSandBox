...
document.addEventListener('DOMContentLoaded', () => {
  ...
  inputDesc.addEventListener('mouseleave', (event) => { // if the user moves the mouse outside the input box
    exitDescriptionEdit(event.target, false);           // treat is like escape was pressed
  });
  ...
  btnSaveReview.addEventListener('click', (event) => {
    event.preventDefault();     // dont perform default button action ********************
...
function toggleDescriptionEdit(desc) {
  const textBox = desc.nextElementSibling;  // get a reference to the next sibling (input box)
  ...
  textBox.focus();                          // give the input box focus(ie put the cursor on it)
}
