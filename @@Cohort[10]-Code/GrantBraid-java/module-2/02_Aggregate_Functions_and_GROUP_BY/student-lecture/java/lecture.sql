...
SELECT name, population
from country
order by 2              -- the column number in the SELECT may be used for the ORDER BY
;
...
LIMIT 10                                -- LIMIT the number of rows from the result
;
...
SELECT name || ', ' || district as city_name_state
FROM city
where district in ('Washington', 'California', 'Oregon')
