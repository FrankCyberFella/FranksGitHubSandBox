...
public class JDBCCityDAOIntegrationTest {
	...
	@Before // Before each test run this method
	public void setup() {
	...
	public void save_new_city_and_read_it_back() throws SQLException {
		...
		dao.save(theCity);	// test the save() in JdbcDAO
		City savedCity = dao.findCityById(theCity.getId());
		
		assertNotEquals(null, theCity.getId());		// Ensure the row is actuallly in the table
		assertCitiesAreEqual(theCity, savedCity);	// Verify the City we retrieve from table has the same values as
													// what we INSERT
	}
	...
	public void returns_cities_by_country_code() {
		...
		dao.save(theCity);
		List<City> results = dao.findCityByCountryCode(TEST_COUNTRY);  // Noth the mehtod returns a list
		
		assertNotNull(results);						//Ensure the row is actually in the table
		assertEquals(1, results.size());			//Make sure these is only one row
		City savedCity = results.get(0);			//Get the first City object in the List returned by find city
		assertCitiesAreEqual(theCity, savedCity);	// Verify the City retreived is the one we created and saved
	}
	...
	private void assertCitiesAreEqual(City expected, City actual) {	// Method used just for testing
