...
on a.actor_id = film_actor.actor_id -- join condition
inner join film f 
...
select sum(payment.amount) as total_sales_amount, avg(payment.amount) as average_payment, count(rental), store.store_id, address
from address
join store
