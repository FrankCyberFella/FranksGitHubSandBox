...
;
rollback; -- undo the changes made since last commit/rollback
...
create table USA_INFO (like country); --create new table that resembles country table

insert into USA_ INFO -- add to the USA_INFO table
...
begin transaction;
update country
set capital = (Select id from city where name = 'Houston') -- using subquery to find code for Houston
, headofstate = 'Jason Lawrie'
where code = 'USA'
;
select capital , headofstate
from country
where capital = (Select id from city where name = 'Houston');
commit;
