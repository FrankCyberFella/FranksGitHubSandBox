...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		...
		//animalMap.get(animalName.toLowerCase()); converts to lowercase
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		if ((petersMoney >= 50 * 100) && (paulsMoney >= 100 * 100)) {
			peterPaul.put("Peter", petersMoney - (petersMoney / 4));
			peterPaul.put("Paul", paulsMoney - (paulsMoney / 4));
			peterPaul.put("PeterPaulPartnership", paulsMoney / 4 + petersMoney / 4);
		}
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		String character1 = "First Character";
		String character2 = "Second Character";
				
		for (String Return : words) {
		character1 = Return.charAt(0) + "";
		character2 = Return.charAt(Return.length()-1) + "";
		
		character.put(character1, character2);
	...
	public Map<String, Integer> wordCount(String[] words) {
		...
		for (String stringName : words) {
			result.put(stringName, 0);		
		}
		for (String stringName : words) {
			result.put	(stringName, result.get(stringName) + 1);	
		}
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		for (Integer intName : ints) {
			result.put(intName, 0);
		}
		for (Integer intName : ints) {
			result.put(intName, result.get(intName) + 1);
		}
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		Map<String, Integer> result = new HashMap<String, Integer>();
		Map<String, Boolean> resultFinal = new HashMap<String, Boolean>();
		...
		for (String stringName : words) {
			result.put(stringName, result.get(stringName) + 1);
			if (result.get(stringName) >= 2) {
				booleanName = true;
		}
			else {
				booleanName = false;
			}
			resultFinal.put(stringName, booleanName);
