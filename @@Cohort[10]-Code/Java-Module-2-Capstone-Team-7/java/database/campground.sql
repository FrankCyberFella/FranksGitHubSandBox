...
SELECT * FROM reservation WHERE (from_date BETWEEN ? AND ?) AND reservation.site_id IN(SELECT site.site_id FROM site WHERE site.site_id = reservation.site_id);

SELECT (daily_fee * ?) FROM campground WHERE campground.campground_id = ?;
