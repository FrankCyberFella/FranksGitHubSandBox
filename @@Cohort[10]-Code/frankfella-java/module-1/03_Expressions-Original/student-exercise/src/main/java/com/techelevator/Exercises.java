...
public class Exercises {
	...
	public boolean posNeg(int a, int b, boolean negative) {
		...
		boolean oneNegOnePos = (a < 0)  ^ (b < 0);   // true of one is negative but not both
		boolean bothNeg      = (a < 0)  && (b < 0);  // true of both are negative
		boolean bothPos      = (a >= 0) && (b >= 0); // true of both are positive
		
		if (oneNegOnePos && negative) {
			return false;
		}
		else {
			if (oneNegOnePos && negative) {
