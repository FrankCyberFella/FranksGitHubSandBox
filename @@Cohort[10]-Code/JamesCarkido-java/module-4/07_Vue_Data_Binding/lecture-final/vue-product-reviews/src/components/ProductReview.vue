...
<!-- create one of the following divs for each element in the review array.  says generate this tag and all of its associated children -->
        <div class="review" v-for="review in reviews" v-bind:key="review.id">
            ...
            <div class="rating">
                <img src="../assets/star.png" v-bind:title="review.rating + ' Star Review'" class="ratingStar" v-for="n in review.rating" />
                                           <!-- adding title to image-->    <!-- give me the # of images that are indicated by the rating-->
                                                                            <!-- says give me this star, this many times -->
            </div>
            <h3>{{ review.title }}</h3>
...
export default {
    ...
    computed: {
        averageRating(vm) {
            ...
            }, 0); // initial value in currentSum set to 0 rather than the value in the first element of the array
            return sum / vm.reviews.length;
        },
        numberOfOneStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 1); // go find me the number of reviews with a rating of 1
        },
        numberOfTwoStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 2);// go find me the number of reviews with a rating of 2
        },
