<!-------------------------------------------------------------->
<!-- REST APIs - SaveReview.vue - Lecture Final               -->
<!-------------------------------------------------------------->
<template>
...
export default {
  ...
  methods: {
    ...
    createReview() {
      fetch(this.apiURL,{  // asynchronously call the api
        method: 'POST',    // using an HTTP post - create or add a reivew
        headers: {         // add to the request header
          "Content-Type": "application/json"  // tell API the data is an application/json form
        },
        body: JSON.stringify(this.review)     // convert the JavaScript Objectto json in the body of the request
      })                                      // make sure that the data is in the body of the post
      .then((response) => {                   // wait for the fetch to finish
        if(response.ok) {                     // if status code is 200 (ok)
          this.$emit('showReviews');          // trigger showReviews event (custom event)
        }
      })
