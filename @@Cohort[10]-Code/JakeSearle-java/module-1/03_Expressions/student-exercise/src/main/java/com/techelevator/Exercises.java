...
<<<<<<< HEAD
		...
		if ((aIsTeen && !bIsTeen) || (bIsTeen && !aIsTeen)) { // ! replaces == false
			return true;
		   } else {
			   return false;
		      }
...
<<<<<<< HEAD
		if (isBirthday == false && speed <= 60) {
			return 0; 
		} else if (isBirthday == false && speed >= 61 && speed <= 80) {
			return 1;
			...
			return 1;
		} else if (isBirthday == true && speed >= 86) {
			return 2;
		} else {
			return 0;
		}
