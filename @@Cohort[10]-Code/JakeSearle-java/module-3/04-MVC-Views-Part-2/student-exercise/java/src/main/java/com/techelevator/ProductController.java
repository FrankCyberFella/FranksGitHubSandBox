...
public class ProductController {
	...
	@RequestMapping("/") // this is the URL path this code will handle 
	public String handleRootPath(HttpServletRequest request) { // receive the request from server
		request.setAttribute("productList", getProducts()); // create entry in request with whatever getProducts() returns
		return "productList"; 								// run the productList.jsp
	}														// return the name of the .jsp file to run 
	
	

	@RequestMapping("/productList") // this is the URL path this code will handle 
	public String showProductList(HttpServletRequest request) { // receive the request from server
		request.setAttribute("productList", getProducts()); // create entry in request with whatever getProducts() returns
		return "productList"; // run the productList.jsp
	}
