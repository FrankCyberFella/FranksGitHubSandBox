...
function filterArrayToOnlySingleDigitNumbers(arrayToFilter) {
  return arrayToFilter.filter(
    digit => Math.abs(digit)<10||Math.abs(digit)>99
  );
}
 ...
 */
/**
 * This method converts a temperature from C to F or F to C
 * 
 * Returns NaN if temperature is not a number
 * Converts F to C by default
 * @param {number} temperature - the number value of temperature 
 * @param {String} temperatureUnit - Fahrenheit or Celsius
 * @param {boolean} includeUnit  - Return as a string with unit lable?
 * @returns {String} with converted temperature and new unit
 */
