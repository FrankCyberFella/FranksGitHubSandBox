...
--
-- if you omit join conditions the result is a Cartersian Product
-- (each row from one table with each row of the other) - not a join!
--
-- minimum # of conditions for WHERE is the # of tables minus 1
--
--  SELECT columns
--    FROM table1
--         INNER JOIN
...
SELECT last_name
FROM actor
INNER JOIN film_actor ON actor.actor_id = film_actor.actor_id -- join condition
INNER JOIN film ON film.film_id = film_actor.film_id -- join condition
