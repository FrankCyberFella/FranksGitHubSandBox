...
public class Lecture {
	...
	//To define one: List<datatype-to-be-stored> name = new ArrayList<datatype-to-be-stored>();
		...
		for(int i = 0; i < names.size(); i++) {
			System.out.println(names.get(i)); //.get() - retrieve the element at the index
		}
		...
		//foreach loop - always go from the beginning to the end one element at a time
		...
		for(String name : names) { //every time through the loop name will be assigned an element
			System.out.println(name); //print the current value from names (name)
		}
