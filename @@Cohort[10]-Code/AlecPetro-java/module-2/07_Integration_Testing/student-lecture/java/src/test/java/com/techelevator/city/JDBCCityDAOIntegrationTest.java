...
public class JDBCCityDAOIntegrationTest {
	...
	/* Before any tests are run, this method initializes the datasource for testing. Only done once*/
	@BeforeClass
	...
	public void save_new_city_and_read_it_back() throws SQLException {
		...
		assertCitiesAreEqual(theCity, savedCity);	// Verify the City we retrieved from the table has the same values as what we think we INSERT'd
	}
	...
	public void returns_cities_by_country_code() {
		...
		dao.save(theCity);
		List<City> results = dao.findCityByCountryCode(TEST_COUNTRY); // Note the method returns a List
		...
		assertCitiesAreEqual(theCity, savedCity);	// Verify the City object retrieved is the one we created and saved
	}
