...
public class Exercises {
	...
	public List<String> reverseList(List<String> stringList) {
		...
		Stack<String> reverse2 = new Stack<String>();
		for(String elements : stringList) {
			reverse2.push(elements);
		}
		while(reverse2.size() > 0) {
			reverse.add(reverse2.pop());
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		// interleave into result the same number of elements - size are equal
		int i = 0;
		for(i = 0; i < smallerLength ; i++) { //loop for the # of items of listOne
			result.add(listOne.get(i)); 			// get an item from listOne and add it to result
			result.add(listTwo.get(i));				//get an item from listTwo and add it to result
		}
		// handle what if one is smaller than the other - add the remaining items in the longer to result
		
		if(listOne.size() > listTwo.size()) {
			for( ; i < listOne.size(); i++) {
				result.add(listOne.get(i));// add remaining elements from listOne to result
			}
		}
		else {	
			if(listTwo.size() > listOne.size())	{
				for( ; i < listTwo.size(); i++) {
					result.add(listTwo.get(i));// add remaining elements from listOne to result
				}
			}
			// add the remaining elements from listTwo to listCombo
		}
