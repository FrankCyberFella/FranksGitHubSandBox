...
--
-- If you omit jion conditions the result is Cartersian Product
-- each from one table with each row in the other - IT'S NOT A JOIN
-- 
-- Minimum number of conditions on where clause should equal the number of tables -1
--
--  SELECT columns
--    FROM table1
--         INNER JOIN
...
WHERE film.title = 'FINDING ANACONDA'
;
...
SELECT p.*, c.customer_id, c.first_name || ' ' || c.last_name AS name
FROM payment p
  INNER JOIN customer c ON p.customer_id = c.customer_id
WHERE p.payment_id = 16666
;
