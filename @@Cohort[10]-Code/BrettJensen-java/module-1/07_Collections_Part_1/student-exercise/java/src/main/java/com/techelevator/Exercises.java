...
public class Exercises {
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		List<String> intAsString = new ArrayList<String>();
		for(Integer el: integerArray) {
			if(el % 3 == 0 && el % 5 == 0) {
				intAsString.add("FizzBuzz");
			} else if (el % 3 == 0 && el % 5 != 0) {
				intAsString.add("Fizz");
			} else if (el % 3 != 0 && el % 5 == 0) {
				intAsString.add("Buzz");
			} else {
				intAsString.add(el.toString());
			}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> intCombo = new ArrayList<Integer>();
		int arrayLength = 0;
		if(listOne.size() > listTwo.size()) {
			arrayLength = listOne.size();
		} else {
			arrayLength = listTwo.size(); 
		}
		for(int i = 0; i < arrayLength; i++) {
			if(i < listOne.size()) {
				intCombo.add(listOne.get(i));
			}
			if(i < listTwo.size()) {
				intCombo.add(listTwo.get(i));
			}
		}
		return intCombo;
	}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		for(Integer el: seatNumberList) {
			if(el > 0 && el < 11) {
				oneToTen.offer(el);
			}
			if(el > 10 && el < 21) {
				elevenToTwenty.offer(el);
			}
			if(el > 20 && el < 31) {
				twentyOneToThirty.offer(el);
			}
		}

		newArray.addAll(oneToTen);
		newArray.addAll(elevenToTwenty);
		newArray.addAll(twentyOneToThirty);
