...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		for (String eachFruit : stringArray) {
			fruit.add(eachFruit);
	...
	public String[] list2Array(List<String> stringList) {
		...
		String[] fruit = new String[stringList.size()]; // establish array and size of array
		int i = 0;
	...
	/*
	 * for (int i = 0; i < stringList.size(); i++) { // regular for loop fruit[i] =
	 * stringList.get(i); }
	 * 
	 * 
	 * 
	 ...
	 * return fruit; }
	 * 
	 * /* Given an array of Strings, return an ArrayList containing the same Strings
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for (String checker : stringArray) {
			// int i = 0;
			if (checker.length() != 4) {
				stuff.add(checker);
				// i++;
	...
	public List<String> reverseList(List<String> stringList) {
		...
		Stack<String> stackThem = new Stack<String>();
		List<String> unstackToList = new ArrayList<String>();

		for (String adjectives : stringList) {
			stackThem.push(adjectives);
		}

		while (stackThem.empty() == false) {

			String color = stackThem.pop();
			unstackToList.add(color);
		}

		return unstackToList;
	}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		List<Double> halfDouble = new ArrayList<Double>();

		for (int intToDouble : intArray) {
			halfDouble.add((double) intToDouble / 2);
		}
		return halfDouble;
	}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		List<Integer> doubledVals = new ArrayList<Integer>();

		for (Integer dubs : integerList) {
			if (dubs == intToFind) {
				doubledVals.add(dubs);
			}
		}
		if (doubledVals.size() >= 2) {
			return true;
		}
		return false;
	}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		// need to go thru multiple loops, so regular for loop is better than ForEach.

		// what if one input list is bigger than the other?
		// go thru both lists for the length of the smaller one.
		int smaller; // hold number items in smaller list

		if (listOne.size() < listTwo.size()) {
			smaller = listOne.size();
		} else {
			smaller = listTwo.size();

		} // interleave elements
		int i = 0;
		for (i = 0; i < smaller; i++) {
			leaveList.add(listOne.get(i));
			leaveList.add(listTwo.get(i));
		}

		// find out if one is smaller? >> ask if >>
		if (listOne.size() > listTwo.size()) {
			// add extra elements from listOne to leaveList
			...
			if (listTwo.size() > listOne.size()) {
				// add extra elements from listTwo to leaveList
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		Queue<Integer> cue1 = new LinkedList<Integer>();
		Queue<Integer> cue2 = new LinkedList<Integer>();
		Queue<Integer> cue3 = new LinkedList<Integer>();
