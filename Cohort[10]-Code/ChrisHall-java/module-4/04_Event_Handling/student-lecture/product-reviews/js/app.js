...
//  Get a reference to the element to manipulate (getElementById or QuerySelector, which?)
//  Here, qs, because 'description' class, no id
const reviewDesc = document.querySelector('.description');
reviewDesc.addEventListener('click', (event) => {
  changeDescription(event.target)     //  call our function, pass it the element that experienced the event
})
...
function changeDescription(reviewDesc) {
  ...
  //  get a reference to the element to change--here to get to input, it is a next sibling
const textBox = reviewDesc.nextElementSibling;
textBox.classList.remove('d-none');             // remove the do not display, now it will be visible
