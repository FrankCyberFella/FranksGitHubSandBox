...
function sumAllNumbers(numbersToSum) {
  //reduce() will return one value as result of an operation on an array
  // first parameter - sum
  // second an array to process
  return numbersToSum.reduce((sum, anElement) => {
    return sum += anElement;
  }
...
function allDivisibleByThree(numbersToFilter) {
  return numbersToFilter.filter((anElem)  => {  // filter says keep element if function returns return
            return anElem % 3 === 0;  // return true if number is divisible by three -- also could  !(anElem % 3)
  }
  )
}
