...
public class Exercises {
	...
	public int sum13(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			...
			if(nums[i] == 13 && nums[i] < nums.length-1) {
				sum += nums[i] - 13 - nums[i+1];
			}
			if(nums[i] == 13) {
				sum += nums[i] - 13;
			}
	...
	public boolean has22(int[] nums) {
		return (nums[0] == 2 && nums[1] == 2 || nums[1] == 2 && nums[2] == 2);
	}
	...
	public boolean lucky13(int[] nums) {
		return (nums[0] != 1 && nums[0] != 3 && nums[1] != 1 && nums[1] != 3 && nums[2] != 1 && nums[2] != 3); 	
	}
