...
SELECT a.last_name, f.title, f.rating
FROM actor a JOIN film_actor fa
  ON a.actor_id = fa.actor_id
  JOIN film f
...
SELECT count(r.customer_id), c.first_name || ' ' || c.last_name as customer
 FROM customer c JOIN rental r
   ...
   GROUP BY r.customer_id, customer
   ORDER BY 1 DESC
   LIMIT 10
 ;
...
SELECT sum(p.amount), c.first_name || ' ' || c.last_name as customer
...
SELECT s.store_id, a.address, count(r.rental_date), sum(p.amount) as total_sales, avg(p.amount) as avg_per_store
  FROM address a JOIN store s
...
SELECT f.title, count(r.rental_date) as total
...
SELECT c.name, count(p.rental_id) as total
  FROM category c JOIN film_category fc
    ON c.category_id = fc.category_id
       JOIN film f
    ON f.film_id = fc.film_id
...
SELECT count(r.inventory_id) as total, a.first_name || ' ' || a.last_name as name
  FROM actor a JOIN film_actor fa
    ON a.actor_id = fa.actor_id
       JOIN film f
    ON f.film_id = fa.film_id
...
SELECT fa.actor_id, count(rental_date) as total , c.name as comedy
