...
public class Exercises {
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if(peterPaul.get("Paul")<1000) {
			if(peterPaul.get("Peter")>0) {
				transfer = peterPaul.get("Peter")/2;
				paulMoney = peterPaul.get("Paul");
				paulMoney += transfer;
				if(peterPaul.get("Peter")%2==1) {
					transfer += 1;
				}
				peterPaul.put("Paul", paulMoney);
				peterPaul.put("Peter", transfer);
			}
		}
		return peterPaul;
	}
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		if(peterPaul.get("Peter")>=5000) {
			if(peterPaul.get("Paul")>=10000) {
				newPeter = (peterPaul.get("Peter")/4)*3;
				newPaul = (peterPaul.get("Paul")/4)*3;
				peterContr = (peterPaul.get("Peter")/4);
				paulContr = (peterPaul.get("Paul")/4);
				peterPaul.put("Peter", newPeter);
				peterPaul.put("Paul", newPaul);
				peterPaul.put("PeterPaulPartnership", peterContr+paulContr);
			}
		}
		return peterPaul;
	}
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		Map<String, Boolean> twoOrMore = new HashMap<String, Boolean>();
		...
		for (int i=0; i<words.length; i++) {
			...
			if (count>=2) {
				twoOrMore.put(word, true);
			}
			else {
				twoOrMore.put(word, false);
			}
		}
		return twoOrMore;
	}
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse, Map<String, Integer> remoteWarehouse) {
		...
		int newSum = 0;
		Set<String> first = mainWarehouse.keySet();
		Set<String> second = remoteWarehouse.keySet();
		for(String same : first) {
			if(remoteWarehouse.containsKey(same)) {
				newSum = mainWarehouse.get(same) + remoteWarehouse.get(same);
				combined.put(same, newSum);
			}
			else {
				combined.put(same, mainWarehouse.get(same));
			}
		}
		for(String notSame : second) {
			if(!mainWarehouse.containsKey(notSame)) {
				combined.put(notSame, remoteWarehouse.get(notSame));
			}
		}
		return combined;
	}
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		...
		for(int i=0; i<words.length; i++) {
			...
			String ender = words[i].substring(words[i].length()-2, words[i].length());
			int count = -1;
			
			for(int j=0; j<words[i].length()-1; j++) {
				
				if (words[i].substring(j, j+2).equals(ender)) {
				count++;
				}
	...
	public List<String> distinctValues(List<String> stringList) {
		...
		Map<String, String> noDuplicates = new HashMap<String, String>();
		for(String originals : stringList) {
			noDuplicates.put(originals, "");
		}
		Set<String> toArray = noDuplicates.keySet();
		for(String transfer : toArray) {
			noDupes.add(transfer);
		}
		return noDupes;
	}
