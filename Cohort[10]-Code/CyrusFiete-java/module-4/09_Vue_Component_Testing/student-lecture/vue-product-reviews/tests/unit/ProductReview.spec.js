...
describe('ProductReview', () => {
  ...
  describe('Computed Properties', () => {
    ...
    it('should have only filtered reviews in filteredReviews', () => {
      ...
      wrapper.vm.filterReviews[0].reviewer.should.equal('TEST3');
      wrapper.vm.filterReviews[1].reviewer.should.equal('TEST5');
    });

    it('should have all reviews in filteredReviews when filter is 0', () => {
      wrapper.setData({reviews: testReviews, filter: 0});
      wrapper.vm.filteredReviews.length.should.equal(0);
    });
  });
});