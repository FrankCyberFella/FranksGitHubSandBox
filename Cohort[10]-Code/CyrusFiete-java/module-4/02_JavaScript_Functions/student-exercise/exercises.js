 ...
 */
/**
 * Takes in a month, day, and hemisphere, and returns the hemisphere in which it is summer
 * or false if it is not summer anywhere on that month and day
 * 
 * @param {number} month 
 * @param {number} [day=1] day of the month
 * @param {String} [hemisphere='northern'] the hemisphere in question
 * @returns {boolean} true if summer and in which hemisphere, false if not
 * 
 */
 ...
 */
 ...
 function makeHappy(x) {
  return happyWords = x.map((element)=> {
...
function getFullAddressesOfProperties (associativeArrays) {
      return associativeArrays.map((mailingAddresses) => {
        return `${mailingAddresses.streetNumber} ${mailingAddresses.streetName} ${mailingAddresses.streetType} ${mailingAddresses.city} ${mailingAddresses.state} ${mailingAddresses.zip}`;
      });
}
