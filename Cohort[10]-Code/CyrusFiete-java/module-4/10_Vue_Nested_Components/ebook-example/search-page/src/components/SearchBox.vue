...
<!-------------------------------------------------------------->
<template>
    <form v-on:submit.prevent="performSearch"> <!-- on submit, don't perform default action, but performSearch instead -->
    ...
    methods: {
        ...
        performSearch() {
            ...
            return fetch(this.searchUrl + this.search + '?format=json').then((response) => {
                 ...
                 */
                this.$emit('search-results', results); //notify parent that we have the data
            });
        }
    }
}
