...
public class ProductController {
	...
	@RequestMapping("/") //the URL path this code will handle
	public String handleRootPath(HttpServletRequest request) { // Receive the request from server
		request.setAttribute("productList", getProducts()); // Create an entry in the request with whatever getProducts() returns
		return "productList"; //Run the productList.jsp (return the name of the jsp file)
	}						  //Return the name of the .jsp file to run as a String
	
	@RequestMapping("/productList") //the URL path this code will handle
	public String showProductList(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productList";
	}
	
	@RequestMapping("/productTiles") //the URL path this code will handle
	public String showProductTiles(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productTiles";
	}
	
	@RequestMapping("/productTable") //the URL path this code will handle
	public String showProductTable(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productTable";
	}
	
	@RequestMapping("/productDetail") //the URL path this code will handle
	...
	//For the purpose of this example - the Model is an ArrayList - it may be other things
