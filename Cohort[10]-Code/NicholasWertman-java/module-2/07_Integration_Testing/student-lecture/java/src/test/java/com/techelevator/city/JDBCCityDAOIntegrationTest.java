...
public class JDBCCityDAOIntegrationTest {
	...
	public void save_new_city_and_read_it_back() throws SQLException {
		...
		dao.save(theCity);	// test the save() method in JDBCCityDAO
		City savedCity = dao.findCityById(theCity.getId());
		
		assertNotEquals(null, theCity.getId());		// Ensure that the row is actually in the table 
		assertCitiesAreEqual(theCity, savedCity);	// Verifying that the retrieved city from the table is equivalent to inserted table
	}
