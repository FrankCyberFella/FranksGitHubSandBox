...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		// define a variable or object to hold return data
		...
		// identify the input (already know output)
		// Figure out how to make output from input
		//
		// output: ArrayList of String
		// input: array of Strings
		return fruits; // remember to return the object/variable defined to hold result
	}
	...
	public String[] list2Array(List<String> stringList) {
		String[] newFruitArray = new String[stringList.size()];
		// List<String> string = new ArrayList<String>();
		int counter = 0;
		for (String item : stringList) {

			newFruitArray[counter] = item;
			counter++;
		}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		// Integer number = new Integer("intArray");
		for (int number : intArray) {
			double dividedNumber = (number / 2.0);

			list.add(dividedNumber);
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for (int i : integerArray) {
			...
			if ((i % 5 == 0 && i % 3 != 0)) {
				// Integer buzz = new Integer("Buzz");
				list.add(String.valueOf("Buzz"));
			}
			if ((i % 3 == 0 && i % 5 == 0)) {
				// Integer fizzBuzz = new Integer("FizzBuzz");
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		Queue<Integer> listOneQueue = new LinkedList<Integer>();
		Queue<Integer> listTwoQueue = new LinkedList<Integer>();
		List<Integer> result = new ArrayList<Integer>();

		{
			for (int element : listOne) {
				listOneQueue.add(element);
			}
			for (int element : listTwo) {
				listTwoQueue.add(element);
			}
			{
				//for (int i : listOne) {
					while (listOneQueue.size() > 0 && listTwoQueue.size() > 0) {
						result.add(listOneQueue.poll());
						result.add(listTwoQueue.poll());
					}
					while (listOneQueue.size() > 0) {
						result.add(listOneQueue.poll());
					}
					while (listTwoQueue.size() > 0) {
						result.add(listTwoQueue.poll());
					}
				//}
			}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		// Queue<Integer> rangeOne = new LinkedList<Integer>();
		// Queue<Integer> rangeTwo = new LinkedList<Integer>();
		Queue<Integer> rangeAll = new LinkedList<Integer>();
		...
		// When I used for loop through while's it returned java heap error --> making
		// all of my first 3 if's run through
		// separate loops fixed my issue
//	//	for (int i : seatNumberList) 
		{
			while (rangeAll.size() > 0) {
				boardingList.add(rangeAll.poll());
			}
			while (rangeAll.size() > 0) {
				boardingList.add(rangeAll.poll());
			}
