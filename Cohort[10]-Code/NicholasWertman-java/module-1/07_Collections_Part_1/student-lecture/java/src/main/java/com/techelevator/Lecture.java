...
public class Lecture {
	...
	// To define one: List<datatype-to-be-stored>	name = new ArrayList<datatype-to-be-stored>();
		...
		names.add("Frodo");		//.add() --> adds elements to end of the ArrayList
		names.add("Sam");
		names.add("Pippin");
		names.add("Merry");
		names.add("Gandalf");
		...
		// .size() --> returns the numebr of elements currently in the ArrayList
		//the elements will be returned in the same order they were added
		for(int i = 0; i < names.size(); i++) {
			System.out.println(names.get(i));  //.get() --> retrieves the element at the index
		}
		...
		//foreach loop --> always go from the beginning to end one element at a time 
			// can;t start anywhere but the beginning; can't stop anywhere but the end

		// Let's loop through names again, but this time using a for-each loop
		// for each name in names
		for(String anElement: names) {  // every time through the loop anElement will be assigned an element.
		...
		browserStack.push("http://www.google.com");					//add element to the top
		browserStack.push("http://www.techelevator.com");			//add element to the top
		browserStack.push("http://www.si.com");						//add element to the top
