...
function filterStringArrayForSon(arrayToFilter) {
  return arrayToFilter.filter(
    (name)=> {
      if(name.includes('son')||name.includes('Son')) {
        return name;
      }
 ...
 * @param {number} temperature gives temperature in degrees
 * @param {string} gives celcius if you wish to convert to celcius
 * @param {boolean} defaults to false, whether or not to include the unit
