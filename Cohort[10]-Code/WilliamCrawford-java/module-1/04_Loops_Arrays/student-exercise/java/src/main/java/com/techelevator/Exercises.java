...
public class Exercises {
	 ...
	 * 2, 6]) → true firstLast6([6, 1, 2, 3]) → true firstLast6([13, 6, 1, 2, 3]) →
	 * false
	 */
	 ...
	 * "rotated left" so {1, 2, 3} yields {2, 3, 1}. rotateLeft3([1, 2, 3]) → [2, 3,
	 * 1] rotateLeft3([5, 11, 9]) → [11, 9, 5] rotateLeft3([7, 0, 0]) → [0, 0, 7]
	 */
	...
	public int[] reverse3(int[] nums) {
		/*
		 * int holdValue; = nums[i]; nums[0] = nums[2]; nums[2] = holdValue; return
		 * nums;
		 */
		/*
		 * int forward = 0; int j = nums.length-1; for (int i = 0; i < nums.length / 2;
		 * i++) { holdValue = nums[i]; nums[i] = nums[j]; nums[j] = holdValue;
		 * 
		 * j--; // move the backward to previous element } return nums;
		 */
 ...
 */
	 ...
	 * exist, returning 0 if the array is length 0. sum2([1, 2, 3]) → 3 sum2([1, 1])
	 * → 2 sum2([1, 1, 1, 1]) → 2
	 */
...
//			return c;}
	 ...
	 * somewhere. has22([1, 2, 2]) → true has22([1, 2, 1, 2]) → false has22([2, 1,
	 * 2]) → false
	 */
	public boolean has22(int[] nums) {
	 ...
	 /* 14. Given an array of ints, return true if the
	 * array contains no 1's and no 3's. lucky13([0, 2, 4]) → true lucky13([1, 2,
	 * 3]) → false lucky13([1, 2, 4]) → false
	 */
	 ...
	 * array is exactly 8. sum28([2, 3, 2, 2, 4, 2]) → true sum28([2, 3, 2, 2, 4, 2,
