...
public class Exercises {
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		List <Double> arrayInt2ListDouble = new ArrayListHalf<>();
		
		for(int i = 0; i < (); i++) {
			 
		 }
		
		return arrayListHalf<>; 
	...
	public List<Integer> oddOnly(Integer[] integerArray) {
		...
		for(Integer tempInt : integerArray) {
		if(tempInt %2 != 0)	{
		oddNums.add(tempInt);	
		}
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> interLeavedList = new ArrayList<Integer>();//Define a list to hold result
		//Go through smaller list get item from minList and add to interLeavedList
		//repeat for listTwo/longestList
		//one item at a time beginning to end
		int minListLength = Math.min(listOne.size(), listTwo.size());//determines length of list
		//regular for loop allows us to go through two loops at a time
		for(int i = 0; i < minListLength; i++) {
		interLeavedList.add(listOne.get(i));//add item from listOne to interLeavedList
		interLeavedList.add(listTwo.get(i));//add item from listTwo to interLeavedList
		}
	
		List<Integer> longestList = (listOne.size() > listTwo.size()) ? listOne: listTwo;
