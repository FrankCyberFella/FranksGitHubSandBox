...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		//input: array of strings
		 //return object/variable defined to hold result
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (int names : intArray) {
		Double name1 = new Double(names);
		array1.add(name1 / 2);
		}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		Integer names1 = new Integer(intToFind);
		int counter = 0;
		for (Integer names : integerList) {
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		List<Integer> int1 = new ArrayList<Integer>(); //defines a list to hold result
		//Go through both lists for the number of items(if different sizes, the smaller one)
		...
		int i = 0;
		//interleave into listCombo the same number of elements--size are equal
		for (i = 0; i < smallerLength; i++) {// loop for the # of items of listOne
		int1.add(listOne.get(i));	//get an item from listOne and add it to int1
		int1.add(listTwo.get(i));//get an item from listTwo and add it to int1
		
		}
		
		//handle what if one is smaller than the other--add remaining items in the longer one to int1
		if (listOne.size() > listTwo.size()) {
			
		//add the remaining elements from listOne to int1	
			for (int j = i; j < listOne.size(); j++) {
				int1.add(listOne.get(j));
			}
		}
		else {//listTwo is bigger
			if(listTwo.size() > listOne.size()) {
				//add remaining elements from listTwo to int1
				for (int k = i; k < listTwo.size(); k++) {
					int1.add(listTwo.get(k));
				}
			}
		}
