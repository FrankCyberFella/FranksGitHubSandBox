...
<!-------------------------------------------------------------->
<template>
    <form v-on:submit.prevent="performSearch"> <!-- prevents default action on submit; runs perform search -->
...
export default {
    ...
    methods: {
         ...
         */
        performSearch() {
            // go to searchUrl API with the search value and the parameter to request data in json format
            // .then - wait for the response from API, then return json data in the response
            return fetch(this.searchUrl + this.search + '?format=json').then((response) => {
                return response.json();
            }).then((results) => {//after we processs response - issue a custom event
                /**
                 ...
                 */
                this.$emit('search-results', results); // notify parent data was received
            });
        }
    }
}
