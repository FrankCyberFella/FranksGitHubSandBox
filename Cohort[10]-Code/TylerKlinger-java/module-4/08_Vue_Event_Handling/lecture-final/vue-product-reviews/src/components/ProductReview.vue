...
export default {
    ...
    computed: {
        ...
        filteredReviews(vm) {
            return vm.reviews.filter((review) => {           //current element used in array given to filter function
                return vm.filter === 0 ? true : vm.filter === review.rating;
                //return true if filter = 0     the boolean dependent on whether the value in filter variable = the rooting value in current review
            });
        },
