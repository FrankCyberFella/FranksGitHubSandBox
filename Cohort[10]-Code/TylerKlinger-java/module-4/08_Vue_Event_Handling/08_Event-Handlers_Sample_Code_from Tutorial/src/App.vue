...
<template>
  <div id="app">
  <p> Sample Code with Method Event Handlers</p>
    <a href="#" id="increase" class="btn" v-on:click="increase">Increase</a> //name of the method inside quotes
...
export default {
  ...
  methods: {
    increase() {
      this.counter += 1; // this. means this component's counter is incremented
    },
    decrease() {
      this.counter -= 1; // this. means this component's counter is decremented
    }
  }
};
</script>
...
export default {
  ...
  methods: {
    updateCounter(event) {
      this.counter += event.target.id === "increase" ? 1 : -1; // the id of element clicked is checked
    }                                                          //if "increase" was clicked, 1 otherwise -1
  }
};
</script>
-->
...
export default {
  ...
  methods: {
    updateCounter(operand, event) {
    
      this.counter += event.target.id === "increase" ? operand : 0;
      this.counter += event.target.id === "decrease" ? operand : 0;
    }
  }
};
</script>
