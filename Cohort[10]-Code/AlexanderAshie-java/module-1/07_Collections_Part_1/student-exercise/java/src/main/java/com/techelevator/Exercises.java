...
public class Exercises {
	...
	public List<String> reverseList(List<String> stringList) {
		...
		Stack<String> reversingStack = new Stack<String>();
		List<String> result = new ArrayList<String>();
		
		for (String tempString : stringList) {
			reversingStack.push(tempString);
		}
		
		String tempString = "";
		
		while (reversingStack.size() > 0) {
			tempString = reversingStack.pop();
			result.add(tempString);
		}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		for (Integer tempInt : integerList) {
			if (tempInt == intToFind) {
				intCounter++;
			}
		}
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		for (int i = 0; i < listOne.size() + listTwo.size(); i++) {
			...
			//Check the new ArrayList size against the loop size and break after it is filled
			//As to not have the for loop run longer than necessary
			if(result.size() == (listOne.size() + listTwo.size())) {
				break;
			}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		Queue<Integer> elevThruTwent = new LinkedList<Integer>();
		Queue<Integer> twentOneThruThirt = new LinkedList<Integer>();
		
		for (Integer tempInt : seatNumberList) {
			if (tempInt >= 1 && tempInt <= 10) {
				oneThruTen.offer(tempInt);
			}
			
			if (tempInt >= 11 && tempInt <= 20) {
				elevThruTwent.offer(tempInt);
			}
			
			if (tempInt >= 21 && tempInt <= 30) {
				twentOneThruThirt.offer(tempInt);
			}
			
		}
		
		result.addAll(oneThruTen);
		result.addAll(elevThruTwent);
		result.addAll(twentOneThruThirt);
