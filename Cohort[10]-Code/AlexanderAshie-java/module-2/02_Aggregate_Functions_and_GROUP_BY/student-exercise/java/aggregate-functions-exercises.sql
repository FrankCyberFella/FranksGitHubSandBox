...
-- (19 rows)
        ...
        SELECT name || ', ' || district || ', ' || population FROM city WHERE district IN ('Pennsylvania', 'West Virginia', 'Kentucky', 'Indiana', 'Michigan') ORDER BY district, name;    
...
-- (58 rows)
        ...
        SELECT name || ' (' || code || '), ' || region AS country_and_code FROM country WHERE continent = 'Africa';

-- 3. The per capita GNP (i.e. GNP multipled by 1000000 then divided by population) of all countries in the 
-- world sorted from highest to lowest. Recall: GNP is express in units of one million US Dollars 
-- (highest per capita GNP in world: 37459.26)

        SELECT name, (gnp * 1000000) / population AS gnp_by_pop FROM country WHERE population > 0 AND gnp > 0 ORDER BY gnp_by_pop DESC;
...
-- (smallest difference: 1.00, 1.00, "Ecuador")
        ...
        SELECT name, gnp - gnpold AS non_absolute_gnp, ABS(gnp - gnpold) AS absolute_gnp FROM country WHERE gnp IS NOT NULL AND gnpold IS NOT NULL ORDER BY absolute_gnp DESC;

-- 17. The average population of cities in each country (hint: use city.countrycode)
-- ordered from highest to lowest.
-- (highest avg population: 4017733.0000, "SGP")

        SELECT countrycode, AVG(population) from city GROUP BY countrycode ORDER BY avg DESC;
...
-- (lowest average life expectancy: 52.5719, 37.2, 76.8, "Africa")
        ...
        SELECT AVG(lifeexpectancy), MIN(lifeexpectancy), MAX(lifeexpectancy), continent FROM country WHERE lifeexpectancy IS NOT NULL GROUP BY continent ORDER BY min;
