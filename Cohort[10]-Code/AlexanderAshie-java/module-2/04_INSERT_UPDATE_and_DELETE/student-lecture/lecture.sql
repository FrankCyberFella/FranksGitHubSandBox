...
;
ROLLBACK; --undo or reset the table since the last commit/rollback


BEGIN TRANSACTION;
INSERT INTO countrylanguage
       (language,  countrycode, isofficial, percentage)
VALUES ('Klingon', 'USA', true, 10)
;
...
BEGIN TRANSACTION;
INSERT INTO countrylanguage
VALUES ('USA', 'Klingon', true, 10) -- value must be in the same order as columns in the table without specifying the columns before values
;
...
WHERE countrycode = 'USA'
;
COMMIT; --undo or reset the table since the last commit/rollback
