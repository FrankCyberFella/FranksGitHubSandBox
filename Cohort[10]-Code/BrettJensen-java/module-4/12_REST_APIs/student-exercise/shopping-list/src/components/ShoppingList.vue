...
export default {
    data() {
        return {
            groceries: [],
            API_URL: 'http://5ca5441addab6d0014bc84f1.mockapi.io/api/groceries'
        }
    },
    methods: {
        changeStatus(id,event) {
            ...
            fetch(`${this.API_URL}/${id}`,{
              ...
              body: JSON.stringify(arrIndex)
              })
              .then((response) => {
                if(response.ok) {
                const index = this.groceries.map(item => item.id).indexOf(id);
                this.groceries.splice(index,1);
              }
            })

            const checkMark = event.target.querySelector('i');
