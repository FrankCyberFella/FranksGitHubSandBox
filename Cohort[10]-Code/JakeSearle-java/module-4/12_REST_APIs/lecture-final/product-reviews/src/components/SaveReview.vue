...
export default {
  ...
  methods: {
    ...
    createReview() {
      fetch(this.apiURL,{ // asynchronously calls the API
        ...
        body: JSON.stringify(this.review)    // convert the JS object to json in the body of the request
      })
      .then((response) => {          // wait for fetch to finish
        if(response.ok) {            // if status code is 200 (ok) 
          this.$emit('showReviews'); // then trigger a showReviews event
        }
      })
