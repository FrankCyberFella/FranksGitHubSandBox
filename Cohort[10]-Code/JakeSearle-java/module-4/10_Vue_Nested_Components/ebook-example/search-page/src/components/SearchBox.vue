...
<!-------------------------------------------------------------->
<template>
    <form v-on:submit.prevent="performSearch"> <!-- on submit, don't perform the default action..run performSearch instead -->
...
export default {
    ...
    methods: {
        ...
        performSearch() {
            ...
            // .then - wait for response from API and then return the json data we got
            return fetch(this.searchUrl + this.search + '?format=json').then((response) => {
                 ...
                 */
                this.$emit('search-results', results); // notify the parent we got the data request
            });
        }
    }
}
