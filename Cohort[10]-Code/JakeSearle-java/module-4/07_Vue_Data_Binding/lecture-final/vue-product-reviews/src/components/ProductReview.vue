...
<template>
    <div class="main">
        ...
        <div class="well-display">
            <div class="well">
                <span class="amount">{{ averageRating }}</span> <!-- calling avgRating method to get a value -->
                Average Rating
            </div>
        ...
        <!-- give me one of the following divs for each item in the array, or as many reviews as we get -->
...
export default {
    ...
    computed: {
        averageRating(vm) { //vm is the component thats calling the method
            let sum = vm.reviews.reduce( (currentSum, review) => {
                return currentSum + review.rating;
            }, 0); // initial value in current sum is 0
            return sum / vm.reviews.length;
        },
        numberOfOneStarReviews(vm) {
            return vm.numberOfReviews(vm.reviews, 1); // these call the numberOfReviews method,
        },                                            // passing in the array of reviews, and a number.
    ...
    methods: {
        numberOfReviews(reviews, starType) {
            return reviews.reduce( (currentCount, review ) => { // currentCount is whatever the reduce returns
                return currentCount + ( review.rating === starType ? 1 : 0); // use 1 if current rating === star type, 0 otherwise
            }, 0); 
        }
    }
}
