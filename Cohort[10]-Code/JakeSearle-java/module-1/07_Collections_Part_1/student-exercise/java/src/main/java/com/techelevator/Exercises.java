...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		// define variable or object to hold return type
		...
		return result;  // return the object/variable defined to hold result
	}
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		if(listOne.size() < listTwo.size()) {
			smallLength = listOne.size();
		} 
		else {
			smallLength = listTwo.size();
		}
		
		for (i = 0; i < smallLength; i++) {  //loops for the length(# of items) of listOne. i defined outside of loop. no need to put "int" in loop in this case.
			numbersList.add(listOne.get(i));
			numbersList.add(listTwo.get(i));
		}		
		
		if (listOne.size() > listTwo.size()) { //add remaining elements from listOne to numbersList
			for (; i < listOne.size(); i++)
			numbersList.add(listOne.get(i));
		}
		else {
			if (listTwo.size() > listOne.size()) { //add remaining elements from listTwo to numbersList
