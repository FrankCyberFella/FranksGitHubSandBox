...
-- minimum # of WHERE conditions = # of tables minus 1
--
...
SELECT customer.customer_id, first_name || ' ' || last_name as cust_name
FROM payment
  INNER JOIN
     customer
...
--  (nickname / alias)
SELECT customer.customer_id, first_name || ' ' || last_name as cust_name
FROM payment p  -- assigns correlation name "p" to payment table
  INNER JOIN
     customer c -- assigns correlation name "c" to customer table. Instead of saying customer. and payment. , we can use c. and p. 
   ON payment.customer_id = customer.customer_id
WHERE payment_id = 16666
;
