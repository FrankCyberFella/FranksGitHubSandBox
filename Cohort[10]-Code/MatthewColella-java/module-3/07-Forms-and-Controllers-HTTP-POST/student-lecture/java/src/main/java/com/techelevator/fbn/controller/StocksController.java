...
@Controller
@RequestMapping("/fbn")
@SessionAttributes("stocks") //"stocks" refers to the map variable stocks, not the name "stocks"
public class StocksController {
	@RequestMapping(path={"/stocks"}, method=RequestMethod.GET)
	public String showStocksPage(ModelMap map){
		List<Stock> stocks = getStocks(); //Define a List called stocks loaded with data from getStocks()
		
		map.put("stocks", stocks); //add list called stocks to ModelMap called map
		
		return "fbn/stocksLandingPage"; //jsp name
	}
