...
public class JDBCCityDAOIntegrationTest {
	...
	private static final String TEST_COUNTRY = "XYZ"; //constqant to be used in any test,global variable
	...
	@BeforeClass //things you wanna do once before any test
	...
	@Before //Before each test, put data in database
	public void setup() { //
	...
	@After //After each test, rollback
	...
	public void save_new_city_and_read_it_back() throws SQLException {
		...
		dao.save(theCity); // do the save
		City savedCity = dao.findCityById(theCity.getId());
		
		assertNotEquals(null, theCity.getId());  //assure row exists
		assertCitiesAreEqual(theCity, savedCity); // verify that retrieved city is same at original you defined
	}
	...
	public void returns_cities_by_country_code() {
		...
		dao.save(theCity); // save
		List<City> results = dao.findCityByCountryCode(TEST_COUNTRY);
		
		assertNotNull(results);						//row exists
		assertEquals(1, results.size());			//only 1 row
		City savedCity = results.get(0);			//get first City object in List returned by country code
		assertCitiesAreEqual(theCity, savedCity);   //Verify the retrieved is equal to 
	}
	...
	//method to construct city object
...
//determine if city object is equal
