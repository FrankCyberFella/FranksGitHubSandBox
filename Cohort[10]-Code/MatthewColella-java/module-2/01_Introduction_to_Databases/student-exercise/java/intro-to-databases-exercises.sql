...
SELECT name, ((gnp/population)*1000000) AS per_capita_gnp FROM country WHERE continent = 'Europe';
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT name, (2019 - indepyear) AS years_since_independence FROM country WHERE indepyear IS NOT NULL;