...
public class RTNValidator {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		File inputFile = getInputFileFromUser(); //Define a Field object to represent the file
		try(Scanner fileScanner = new Scanner(inputFile)) { // encapsulated the object called file scanner
	...
	private static File getInputFileFromUser() { // returns a file object for a file that exists
		Scanner userInput = new Scanner(System.in);
		System.out.print("Please enter path to input file >>> ");
		String path = userInput.nextLine();
		System.out.println("Path entered: "+ path + "\n"); //Get the path for the file from user
		File inputFile = new File(path); // Defines a fle object to represent the file
		inputFile.
		
		if(inputFile.exists() == false) { // .exists checks for the existence of a file
