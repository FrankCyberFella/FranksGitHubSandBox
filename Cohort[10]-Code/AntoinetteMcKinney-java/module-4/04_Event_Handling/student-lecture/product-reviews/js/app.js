...
// When the user clicks the descripton they can change
// Show input box when they click it 

// Get a reference to the element to manipulate the DOM 
const reviewDesc = document.querySelector('.description');
reviewDesc.addEventListener('click', (event) => { //event is not standard name
  changeDescripton(event.target) // call our function passing it the element that experienced the event
})

function changeDescripton(reviewDesc){ //same name as descripton as above 
// unhide the input box
// get the data from the input box
// set the descripton element to the data
