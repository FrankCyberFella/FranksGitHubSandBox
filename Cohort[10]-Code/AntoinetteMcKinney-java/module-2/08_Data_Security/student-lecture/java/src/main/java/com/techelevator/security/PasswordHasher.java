...
public class PasswordHasher {
    ...
    private static final int WORK_FACTOR = 100000;//Work_Factor is a constant we know cause its in all caps
    ...
    //USE THIS CODE to SETUP SECURITY
    private Key createKey(String password, byte[] salt) {
        SecretKeyFactory factory = getSecretKeyFactory();
        KeySpec keyspec = new PBEKeySpec(password.toCharArray(), salt, WORK_FACTOR, KEY_LENGTH); //c
        Key key;
        try {
