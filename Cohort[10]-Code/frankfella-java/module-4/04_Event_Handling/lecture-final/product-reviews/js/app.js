...
// save the description and ignore it if hit escape
  ...
  inputDesc.addEventListener('mouseleave', (event) => {  // I fthe user move sthe mouse outside input bix
    exitDescriptionEdit(event.target, false);            // treat it like Escape was pressed
  });
  ...
  btnSaveReview.addEventListener('click', (event) => {
    event.preventDefault();  // Don't perform default button action
    saveReview();            // do this instead
  });
});
...
function toggleDescriptionEdit(desc) {
  const textBox = desc.nextElementSibling;  // Get a reference to the input box
  textBox.value = description;              // Remember the current description
  textBox.classList.remove('d-none');       // Un-hide the input box
  desc.classList.add('d-none');             // Hide the dsescrition
  textBox.focus();                          // Give the input box focus (ie. put the cursot in it)
}
