...
SELECT * 
FROM site 
WHERE campground_id = 1 and site_id NOT IN 
(SELECT site_id
FROM reservation 
WHERE ( ( '2019-02-17' NOT BETWEEN from_date and to_date 
AND '2019-02-23' NOT BETWEEN from_date and to_date 
AND from_date NOT BETWEEN '2019-02-17' and '2019-02-23' 
AND to_date NOT BETWEEN '2019-02-17' and '2019-02-23' ))
);

SELECT *
FROM site
WHERE campground_id = ? AND  site_id NOT IN (SELECT site_id FROM reservation
 WHERE (( to_date BETWEEN ? AND ? ) 
 AND (from_date BETWEEN ? AND ? ) 
 AND ? NOT BETWEEN from_date and ? AND to_date) 
 GROUP BY site_id);
