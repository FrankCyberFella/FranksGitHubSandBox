...
export default {
  ...
  methods: {
    ...
    saveReview() {
      // if reviewID is 0 - add the review, otherwise update review
      this.reviewID === 0 ? this.createReview() : this.updateReview();
    },
    createReview() {
      fetch(this.apiURL,{ // asynchronously call the api
        method: 'POST',   // using http POST - create/add a review
        headers: {        // add to request header
          "Content-Type": "application/json"  // Tell api the data is application/json form
        },
        body: JSON.stringify(this.review) // convert the Javascript object to JSON in the body of the request
      })
      .then((response) => { // wait for the fetch to finish
        if(response.ok) {   // if status code is 200 (OK)
          this.$emit('showReviews');  // trigger a showReviews event
        }
      })
