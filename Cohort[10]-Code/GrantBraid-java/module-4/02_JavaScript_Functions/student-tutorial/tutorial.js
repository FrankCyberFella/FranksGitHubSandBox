...
function mapArrayToDoubleAllNumbers(arrayToDouble) {
  return arrayToDouble.map( (element) => { return element * 2; }
    // WRITE CODE HERE
  );
}
...
function reduceArrayToFindProduct(arrayToMultiply) {
  return arrayToMultiply.reduce( (current, next) => {
    return current * next;
  }
    // WRITE CODE HERE
  );
 ...
 */
 ...
 /**
  * 
  * @param {number} temperature - current temperature in desired units
  * @param {String} temperatureUnit  - chosen unit either 'F' or 'C'
  * @param {boolean} includeUnit - converts the temperature to proper unit
  */
