...
describe('ProductReview', () => {
  ...
  describe('Computed Properties', () => {
    ...
    it('should return only count of one star review from numberOfOneStarReviews', () => {
      wrapper.setData({reviews: testReviews});
      wrapper.vm.numberOfOneStarReviews.should.equal(1);
    });
    it('should return only count of two star review from numberOfTwoStarReviews', () => {
      wrapper.setData({reviews: testReviews});
      wrapper.vm.numberOfTwoStarReviews.should.equal(2);
    });
    it('should return only count of three star review from numberOfThreeStarReviews', () => {
      wrapper.setData({reviews: testReviews});
      wrapper.vm.numberOfThreeStarReviews.should.equal(3);
    });
    it('should return only count of four star review from numberOfFourStarReviews', () => {
      wrapper.setData({reviews: testReviews});
      wrapper.vm.numberOfFourStarReviews.should.equal(0);
    });
    it('should return only count of five star review from numberOfFiveStarReviews', () => {
      wrapper.setData({reviews: testReviews});
    ...
    it('should have only filtered reviews in filteredReviews', () => {
      wrapper.setData({reviews: testReviews, filter: 5});
      wrapper.vm.filteredReviews.length.should.equal(2);
    });
    it('should show all reviews in filteredReviews', () => {
      wrapper.setData({reviews: testReviews, filter: 0});
      wrapper.vm.filteredReviews.length.should.equal(8);
      wrapper.vm.filteredReviews[1].reviewer.should.equal('TEST2');
      wrapper.vm.filteredReviews[4].reviewer.should.equal('TEST5');
    });
