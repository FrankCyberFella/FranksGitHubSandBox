...
public class ProductController {
	...
	@RequestMapping("/productList")  // url path this will handle
	public String showProductList(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productList";  // name of .jsp file
	}
	
	@RequestMapping("/")  // url path this will handle
	public String showSomething(HttpServletRequest request) {
		request.setAttribute("productList", getProducts());
		return "productList";
	}
	...
	public String showProductDetail(HttpServletRequest request) {
		String productId = request.getParameter("productId"); // get value in "productId" parameter
		for(Product p : getProducts()) {                // loop through each product from getProducts()
			if(p.getProductId().equals(productId)) {    // does the current product ID match parameter ID
				request.setAttribute("product", p);     // if it does add the product with that product ID
			}                                           //    to the request object with the key "product"
		}
